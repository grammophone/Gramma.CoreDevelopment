...
namespace Gramma.Inference.Trainer
{
	...
	public class TrainingOptionsDefinition
	{
		...
		private Gramma.Inference.Sentences.WordScoringPolicy wordScoringPolicy;
		...
		public TrainingOptionsDefinition()
		{
			...
			this.SentencesTrainType = Inference.Trainer.SentencesTrainType.Offline;
			...
			this.wordScoringPolicy = Inference.Sentences.WordScoringPolicy.Prioritized;
		...
		public Gramma.Inference.Sentences.WordScoringPolicy WordScoringPolicy
		{
		...
		private Inference.Words.WordClassifierTrainingOptions GetWordTrainingOptions()
		{
			var trainingOptions = new Inference.Words.WordClassifierTrainingOptions();
		...
		private Inference.Sentences.SentenceClassifierTrainingOptions GetSentenceTrainingOptions()
		{
			var trainingOptions = new Inference.Sentences.SentenceClassifierTrainingOptions();
			...
			{
				case Inference.Trainer.SentencesTrainType.Offline:
					trainingOptions.TrainingMethod = Inference.Sentences.SentenceClassifierTrainingMethod.Offline;
					trainingOptions.OfflineTrainingOptions.Regularization = this.CrfWeightsRegularization;
					break;

				case Inference.Trainer.SentencesTrainType.Online:
					trainingOptions.TrainingMethod = Inference.Sentences.SentenceClassifierTrainingMethod.Online;
			...
			switch (this.DecayFunctionType)
			{
				case DecayFunctionType.Linear:
					trainingOptions.OnlineTrainingOptions.OptimizationOptions.DecayFunction = new Gramma.Optimization.DecayFunctions.LinearDecayFunction();
					break;
				
				case DecayFunctionType.Root:
					trainingOptions.OnlineTrainingOptions.OptimizationOptions.DecayFunction = new Gramma.Optimization.DecayFunctions.RootDecayFunction();
					break;
		...
		private IEnumerable<Inference.Words.WordClassifierTrainingOptions> GetWordTrainingOptionsGrid()
		{
			for (double marginSlack = this.MarginSlackStart; marginSlack <= this.MarginSlackEnd; marginSlack *= this.MarginSlackStepFactor)
			{
				for (double stringExponent = this.StringExponentStart; stringExponent <= this.StringExponentEnd; stringExponent *= this.StringExponentStepFactor)
				{
					...
					if (this.IncludeGaussian)
					{
						...
						{
							trainingOptions = new Inference.Words.WordClassifierTrainingOptions();
		...
		private IEnumerable<Inference.Sentences.SentenceClassifierTrainingOptions> GetSentencesTrainingOptionsGrid()
		{
